...
# Variant target: WGAN-GP (gradient penalty in training script).
...
class WGANGenerator1D(nn.Module):
    """1D Generator for vibration signals (outputs in [0,1])."""
    ...
    def __init__(self, latent_dim: int = 128, out_length: int = 1024, base_channels: int = 128):
        ...
        self.out = nn.Sequential(nn.Conv1d(channels[-1], 1, kernel_size=3, padding=1), nn.Sigmoid())
...
class WGANCritic1D(nn.Module):
    ...
    def __init__(self, in_length: int = 4096, base_channels: int = 64):
        super().__init__()
        if in_length % 64 != 0:
...
class VanillaDiscriminator1D(nn.Module):
    ...
    def __init__(self, in_length: int = 1024, base_channels: int = 64):
        super().__init__()
        assert in_length == 1024, "Current discriminator assumes length 1024."
        ...
        self.head = nn.Linear(base_channels * 8 * 16, 1)
