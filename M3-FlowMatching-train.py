...
Diffusion = load_attr_from_file("M3_FlowMatching", "M3-FlowMatching.py", "Diffusion")
from utils import *
from training_time_tracker import TrainingTimeTracker  # 导入训练时间跟踪器

from U_Net import UNet1D
modelName = 'FlowMatching'
...
def trainFDDiffusion(generateLabel=generateLabel, sampleNumber=sampleNumber, noise_steps=noise_steps, epochs=epochs):
    ...
    # training dataset
    trainset = FFTSignalDataset(dataSource=datasets, numOfClass=generateLabel, numOfData=sampleNumber, lengthOfSample=sampleLength, fs=12000)
    ...
    for i in range(3):
        index = random.randint(0, len(trainset.dataset) - 1)
    ...
    # For stability: set global seeds optional (uncomment to reproduce)
    # seed = 42
    ...
    for epoch in range(epochs + 1):
        ...
        for step, batch in enumerate(trainloader):
            ...
            # --- 用连续浮点时间采样（提高泛化） ---
            # sample continuous t in [1, noise_steps-1] (float)
...
def generateSamples(generateLabel=generateLabel, generateNumber=generateNumber, generateBatchsize=generateBatchsize):
    ...
    ckpt_path = r"{}/{}_{}/ckpt.pt".format(modelSavingPath, modelName, generateLabel)
...
def trainAndValidation(num_epochs, batch_size, count):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 50 == 0:
            print(f'Epoch [{epoch}/{num_epochs}], Loss: {epoch_loss:.3f}')
    ...
    print("验证数据集的长度： ", len(validation_dataset))
    ...
    plot_confusion_matrix(confusion_matrix, classes=['{}'.format(i) for i in range(numOfClasses)],savingPath=r"./confusionmatrix/", name=r"{}_{}".format(modelName, count))
...
if __name__ == '__main__':
    ...
    if retrain:
        ...
        for i in range(0, numOfClasses):
            ...
            print("{}__Class FM_DM Training Finished.".format(i))
    ...
    if regenerate:
        print("============================ FM UNet Sample Generation ==========================")
        phase_start = time.time()
        for i in range(0, numOfClasses):
            generateSamples(generateLabel=i)
            print("{}__Class FM_DM Sample Generation Finished.".format(i))
        phase_time = time.time() - phase_start
